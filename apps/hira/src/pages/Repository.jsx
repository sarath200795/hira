  ...
  return (
    <div>
      <PageHeader tour="repository-header" title="Repository" subtitle="Browse and filter all risk assessments." icon={FolderOpen}>
        <Link to="/apps/hira/create" className="btn-primary"><FilePlus2 size={16} /> New assessment</Link>
      </PageHeader>
      ...
      {loading && assessments.length === 0 ? (
        ...
        <EmptyState
          ...
          action={!filtersActive && <Link to="/apps/hira/create" className="btn-primary mt-2"><FilePlus2 size={16} /> Create assessment</Link>}
        />
      ) : (
            ...
            <table className="w-full text-sm">
              ...
              <tbody className="divide-y divide-ink-100">
                {filtered.map((a) => {
                  const r = topRisk(a)
                  return (
                    <tr key={a.id} className="group cursor-pointer transition hover:bg-clay-100/40" onClick={() => navigate(`/apps/hira/assessment/${a.id}`)}>
                      ...
                      <td className="px-4 py-3" onClick={(e) => e.stopPropagation()}>
                        <div className="flex items-center justify-end gap-1 opacity-100 transition-opacity duration-150 sm:opacity-0 sm:group-hover:opacity-100 sm:group-focus-within:opacity-100">
                          <Link to={`/apps/hira/assessment/${a.id}`} className="rounded-lg p-2 text-ink-500 shadow-clay-sm transition hover:bg-clay-100 hover:text-ink-800" title="View"><Eye size={16} /></Link>
                          ...
                          <Link to={`/apps/hira/create/${a.id}`} className="rounded-lg p-2 text-ink-500 shadow-clay-sm transition hover:bg-clay-100 hover:text-ink-800" title="Edit"><Pencil size={16} /></Link>

...
export default function Dashboard() {
  ...
  const kpis = [
    ...
    { label: 'Open Actions', value: summary.openActions, color: '#2563eb', icon: CalendarClock, to: '/apps/hira/action-tracker' },
    { label: 'Overdue Actions', value: summary.overdueActions, color: '#b91c1c', icon: Clock, to: '/apps/hira/action-tracker' },
  ]
    ...
    <div>
      ...
      {!hasAny && !loading ? (
        <EmptyState
          ...
          action={<Link to="/apps/hira/create" className="btn-primary mt-2"><FilePlus2 size={16} /> Create assessment</Link>}
        />
      ) : (
        <>
          {/* Filter bar */}
          ...
          <div data-tour="dash-charts" className="grid gap-4 lg:grid-cols-3">
            ...
            <ChartCard title="Control Actions (CAPA)" subtitle={`Additional-control status · ${summary.overdueActions} overdue`}>
              ...
              <div className="mt-2 text-center">
                <Link to="/apps/hira/action-tracker" className="text-xs font-semibold text-brand-600 hover:underline">Open Action Tracker →</Link>
              </div>
            </ChartCard>

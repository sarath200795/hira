...
const outFile = path.join(outDir, 'hira.mp4')
const silentFile = path.join(outDir, 'hira.silent.mp4')
...
const t0 = Date.now()
const rec = await startScreencast(page, path.join('/tmp', 'frames-hira'))
initAudio(path.join('/tmp', 'audio-hira'), t0)
...
if (res) {
  ...
  const wav = path.join('/tmp', 'audio-hira', 'narration.wav')
